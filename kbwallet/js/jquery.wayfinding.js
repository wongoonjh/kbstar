...
(function ($) {
	...
	$.fn.wayfinding = function (action, options, callback) {
		...
		function buildDataStore(mapNum, map, el) {
			...
			$('#Paths line, #chk line, #inna line', el).each(function () {
				path = {};
...
//				path.mapNum = mapNum; // index of floor in array 1
				...
				path.x = Math.round($(this).attr('x1'));
				path.y = Math.round($(this).attr('y1'));
				path.d = [];
				path.m = Math.round($(this).attr('x2'));
				path.n = Math.round($(this).attr('y2'));
				path.e = [];
				path.l = Math.sqrt(Math.pow(path.x - path.m, 2) + Math.pow(path.y - path.n, 2));

				path.c = []; //other paths
				path.q = []; // connected portals
				path.id = $(this).attr('id'); 
			...
			$('#Doors line', el).each(function () { // index, line
				x1 = Math.round($(this).attr('x1'));
				y1 = Math.round($(this).attr('y1'));
				x2 = Math.round($(this).attr('x2'));
				y2 = Math.round($(this).attr('y2'));
				doorId = $(this).attr('id');
//				console.log("================  doors")
//				console.log(doorId)
				$.each(dataStore.p[mapNum], function (index, segment) {
					if (map.id === segment.floor && ((Math.round(segment.x) === x1 && Math.round(segment.y) === y1) || (Math.round(segment.x) === x2 && Math.round(segment.y) === y2))) {
						segment.d.push(doorId);
					} else if (map.id === segment.floor && ((Math.round(segment.m) === x1 && Math.round(segment.n) === y1) || (Math.round(segment.m) === x2 && Math.round(segment.n) === y2))) {
...
dataStore.p[0].forEach(function(data){
//	console.log(data)
		...
		function setEndPoint(endPoint, el) {
			...
			if (options.showLocation) {
				...
				var startMap = el.children().has($('#' + escapeSelector(endPoint)));
		...
		function enabledCheckIn(chkins) { 
			
			chkins.forEach(function(chk){
				if ( options.defaultStyle == '' ) options.defaultStyle = $("#w3f"+chk).prop('style').fill;
				$("#w3f"+chk).prop('style', options.enabledStyle)	
				options.changedElement.push("w3f"+chk);
			})
		}	

		function  disabledCheckIn() {
			options.changedElement.forEach(function(elId) {
				$("#"+elId).css({fill : options.defaultStyle})	
			})
			options.changedElement = [];
		}
...
//                                      drawing[level][i+1].type = "L";
		...
		this.each(function () {
			...
			if (action && typeof (action) === 'string') {
				...
				case 'enabledCheckIn' :
					enabledCheckIn (passed);
					break;
				case 'disabledCheckIn' :
					disabledCheckIn();
					break;	
				/**
				 * @function animatePath
